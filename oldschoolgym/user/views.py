...
from .serializers import (
    ...
    MyUserSerializerToUpdate,
    MyUserSerializerToView,
...
class UserAPI(APIView):
    ...
    @swagger_auto_schema(
        operation_description="To change user`s fields. Returns a updated user.",
        responses={200: MyUserSerializer},
        request_body=MyUserSerializerToUpdate,
    ...
    def put(self, request, format=None):
        try:
            ...
            return Response(
                {"id": "User does not exist"}, status=status.HTTP_404_NOT_FOUND
            )
    ...
    @swagger_auto_schema(
        operation_description="To change user`s fields (partial). Returns a updated user.",
        responses={200: MyUserSerializer},
        request_body=MyUserSerializerToUpdate,
        manual_parameters=[get_query_params("user_id", "User id to update")],
    )
    def patch(self, request, format=None):
        ...
        user_serialized = MyUserSerializerToUpdate(user, request.data, partial=True)
        if user_serialized.is_valid():
            user_serialized.save()
            return Response(user_serialized.data, status=status.HTTP_200_OK)
        else:
    ...
    @swagger_auto_schema(
        operation_description="To delete user. Nothing to return.",
        responses={204: {}},
        manual_parameters=[get_query_params("user_id", "User id to delete")],
    )
    ...
    @swagger_auto_schema(
        operation_description="To create user. Returns a default user with unconfirmed email.",
        request_body=MyUserSerializer,
        responses={200: MyUserSerializer},
    )
...
@swagger_auto_schema(
    method="post",
    request_body=ConfirmMailSerializer,
...
def confirm_email(request):
    ...
    if serialized_data.is_valid():
        if request.user.verifying.code == serialized_data.data["code"]:
            ...
            return Response(
                {"code": "Code to confirm is uncorrect!"},
